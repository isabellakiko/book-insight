...
- **脚本分析 + 前端展示**：后端脚本积累数据，前端纯可视化

**开发进度**: MVP 核心功能已实现
**架构模式**: 脚本分析 → JSON 存储 → 前端展示（无实时 AI 调用）
...
5. **分析采样** - 默认采样 30 章（计划提升至 100 章，见修复计划文档）
...
- 前端 React 页面框架
- RAG 系统基础实现
- AI 任务模块（章节分析、人物提取、按需分析）
- 前端设计系统（4 套主题：Light/Dark/Sepia/Midnight）
- 人物详情页 CharacterDetail V4（左侧目录+右侧Tab）
- 人物增量分析系统（脚本 CLI + SSE 流式）
- 赵秦数据 100% 覆盖率分析

### 📋 待执行

- 数据完整性修复（见 `docs/深度排查/数据完整性修复计划-2025-12-11.md`）
- RAG 问答功能测试
...
├────────│────────────────────────────────────────────────────┤
│        │ 只读展示（不触发 AI 分析）                           │
│  Dashboard  │  Characters  │  CharacterDetail  │  Timeline   │
│              └──────────────────────────────────┘            │
│              ┌──────────────────────┐                        │
│              │   Zustand Store     │  ← UI 状态              │
...
**核心原则**: 前端只读取已生成的 JSON 文件，不触发 AI 分析
